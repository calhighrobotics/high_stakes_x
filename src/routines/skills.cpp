...
void Autonomous::Skills(Intake &intake, Latch &latch, DistanceSensor &distance, LadyBrown &ladybrown) {
   ...
   // chassis.turnToPoint(24, 13.5, 1000, {.forwards = false});
   // //chassis.moveToPoint(17.5, 15, 1850, {.forwards = false, .earlyExitRange=0.5}, false);
   // chassis.moveToPoint(24, 13.5, 2000, {.forwards = false, .minSpeed=25, .earlyExitRange=12});
   // chassis.moveToPoint(24, 14.5, 2000, {.forwards=false, .maxSpeed=30});
   ...
   // chassis.turnToPoint(43, 64.5, 800, {.forwards=false});

   // //Actually move there
   // chassis.moveToPoint(43, 64.5, 1500, {.forwards=false}, true);

   // chassis.waitUntilDone();

   // //Turn to the Neutral wall stake.
   // //chassis.turnToPoint(79, 63.5, 750);
   // chassis.turnToPoint(72, 64.5, 800);
   // // Move there
   // chassis.moveToPoint(63, 64.5, 1050, {.forwards=true, .maxSpeed=70}, true);
   ...
   // chassis.moveToPoint(47, 64.5, 750, {.forwards = false, .maxSpeed = 75});
   // IntakeMotor.move_velocity(600);
   // chassis.waitUntil(5);
   // ladybrown.MoveToPoint(LadyBrown::BASE_STATE);
   ...
   // chassis.moveToPoint(47, -3, 2500, {.forwards = true, .maxSpeed = 45, .earlyExitRange=2});
   ...
   chassis.setPose(new_x, new_y, -90);
   chassis.moveToPoint(chassis.getPose().x-5, chassis.getPose().y, 600, { .minSpeed=60, .earlyExitRange=2});

   //chassis.moveToPose(0, 15, -90, 1500, {.horizontalDrift = 8, .minSpeed = 15});
   
   chassis.turnToPoint(-24, 12.5, 1000, {.forwards=false});
   chassis.moveToPoint(-24, 12.5, 5000, {.forwards = false, .minSpeed=50, .earlyExitRange=26});
   chassis.moveToPoint(-24, 12.5, 1000, {.forwards = false, .maxSpeed=45});
   ...
   ladybrown.MoveToPoint(LadyBrown::LOAD_STATE);
   chassis.turnToPoint(-43, 60, 800, {.forwards=false});

   //Actually move there
   chassis.moveToPoint(-43, 60, 1500, {.forwards=false}, true);
   ...
   chassis.turnToPoint(-78, 60, 660);


   // Move there
   chassis.moveToPoint(-72.5, 60, 1020, {.forwards=true, .maxSpeed=50}, true);
   ...
   //Move back to the 5th tile edge, and restart the intake 15 inches into the movement.
   chassis.moveToPoint(-47, 60, 750, {.forwards = false, .maxSpeed = 75});
   IntakeMotor.move_velocity(600);
   chassis.waitUntil(5);
   ladybrown.MoveToPoint(LadyBrown::BASE_STATE);
   ...
   //Move to the top edge of the first tile, preparing for alignment. Then move back into the wall to use the distance sensor to reorient.
   chassis.moveToPoint(-14, 110.5, 7000, {.forwards = true, .maxSpeed = 100, .minSpeed=40, .earlyExitRange=6},  true);
   //chassis.moveToPoint(70, 132.5, 1500);

   chassis.moveToPose(-70, 120, -90, 5000);
